...
<style>
  ...
  .rrc-logo-text { font-size: 1.6rem; font-weight: 700; color: #ffffff; letter-spacing: 2px; }
  .rrc-logo-accent { color: #e85d04; }
...
# ── Session state ─────────────────────────────────────────────────────────────
if "etap_uploaded" not in st.session_state:
    st.session_state.etap_uploaded = False
if "sld_uploaded" not in st.session_state:
...
if st.session_state.page == "Upload":
    ...
    with col1:
        ...
        <div class='rrc-card'>
            ...
            <div class='rrc-card-title'>⚙️ ETAP Losses File</div>
        </div>
        """, unsafe_allow_html=True)

        etap_file = st.file_uploader(
            "Upload ETAP Excel file (.xlsx)",
            type=["xlsx", "xls"],
            key="etap_uploader",
            help="5-sheet Excel file with circuit components, load and no-load losses"
        )
        if etap_file:
            st.session_state.etap_uploaded = True
            st.success(f"✅ **{etap_file.name}** uploaded successfully!")
            st.markdown(f"""
            <div style='background:#0d1117; border:1px solid #21262d; border-radius:6px; padding:12px; margin-top:8px; font-size:0.82rem; color:#8b949e;'>
                📄 File: <span style='color:#e6edf3;'>{etap_file.name}</span><br>
                📦 Size: <span style='color:#e6edf3;'>{etap_file.size / 1024:.1f} KB</span><br>
                📋 Expected sheets: <span style='color:#3fb950;'>Circuit Components · LV Losses · MV Losses · Aux Losses · Summary</span>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown("""
            <div style='font-size:0.82rem; color:#8b949e; margin-top:8px; padding:12px; background:#0d1117; border-radius:6px; border:1px solid #21262d;'>
                📋 <b style='color:#c9d1d9;'>Expected format:</b><br>
                · Sheet 1: Circuit Components<br>
                · Sheet 2: LV Load / No-Load Losses<br>
                · Sheet 3: MV Load / No-Load Losses<br>
                · Sheet 4: Auxiliary Losses<br>
                · Sheet 5: Summary Data
            </div>
            """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        if sld_file:
            ...
            st.markdown(f"""
            <div style='background:#0d1117; border:1px solid #21262d; border-radius:6px; padding:12px; margin-top:8px; font-size:0.82rem; color:#8b949e;'>
                📄 File: <span style='color:#e6edf3;'>{sld_file.name}</span><br>
                📦 Size: <span style='color:#e6edf3;'>{sld_file.size / 1024:.1f} KB</span><br>
    ...
    pct = files_done * 50
    st.markdown(f"""
    <div style='margin-bottom:8px; display:flex; justify-content:space-between; font-size:0.82rem;'>
        <span style='color:#8b949e;'>Upload Progress</span>
        <span style='color:#e85d04; font-weight:600;'>{files_done}/2 files</span>
    </div>
    ...
    if files_done == 2:
        ...
        if st.button("▶  Proceed to Loss Analysis", use_container_width=False):
            st.session_state.page = "LV Losses"
            st.rerun()

# ══════════════════════════════════════════════════════════════════════════════
# SHARED LOSS PAGE TEMPLATE
# ══════════════════════════════════════════════════════════════════════════════
def loss_page(step, title, subtitle, sheet_num, components):
    ...
    if not st.session_state.etap_uploaded:
        st.warning("⚠️ Please upload the ETAP file first (Step 1 — Upload).")
        return

    st.markdown(f"""
    <div class='rrc-card'>
        <div class='rrc-card-header'>Components Detected</div>
        <div class='rrc-card-title'>{title}</div>
    </div>
    """, unsafe_allow_html=True)

    st.info(f"📂 Data will be parsed automatically from Sheet {sheet_num} of your ETAP file.")

    st.markdown("""
    <div class='metric-row'>
        <div class='metric-card orange'>
            <div class='metric-label'>Load Losses</div>
            <div class='metric-value'>—<span class='metric-unit'>kW</span></div>
        </div>
        <div class='metric-card blue'>
            <div class='metric-label'>No-Load Losses</div>
            <div class='metric-value'>—<span class='metric-unit'>kW</span></div>
        </div>
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class='metric-row'>
        <div class='metric-card orange'>
            <div class='metric-label'>LV Losses</div>
            <div class='metric-value'>—<span class='metric-unit'>kW</span></div>
        </div>
        <div class='metric-card blue'>
            <div class='metric-label'>MV Losses</div>
            <div class='metric-value'>—<span class='metric-unit'>kW</span></div>
        </div>
        <div class='metric-card green'>
            <div class='metric-label'>Aux Losses</div>
            <div class='metric-value'>—<span class='metric-unit'>kW</span></div>
        </div>
        <div class='metric-card highlight'>
            <div class='metric-label'>⚡ Total Losses</div>
    ...
    st.markdown("""
    <div class='rrc-card'>
        <div class='rrc-card-header'>Loss Breakdown</div>
        <div class='rrc-card-title'>Component-Level Summary Table</div>
    </div>
    """, unsafe_allow_html=True)
