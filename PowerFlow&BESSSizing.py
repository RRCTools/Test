...
with st.sidebar:
    ...
    contract_power = st.number_input("Contract Power (MW)", 200.0, min_value=1.0, max_value=5000.0, step=10.0)
    contract_pf = st.number_input("Contract Power Factor", 0.95, min_value=0.5, max_value=1.0, step=0.01)

    st.markdown('<div class="section">POI Voltage</div>', unsafe_allow_html=True)
    v_max = st.number_input("Max (pu)", 1.05, min_value=1.0, max_value=1.15, step=0.01)
    v_min = st.number_input("Min (pu)", 0.95, min_value=0.8, max_value=1.0, step=0.01)
    v_calc = st.number_input("This calculation (pu)", 1.00, min_value=0.8, max_value=1.15, step=0.01)

    st.markdown('<div class="section">Main Power Transformer (MPT)</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    n_mpt = c1.number_input("# MPTs", 1, min_value=1, max_value=10)
    s_mpt = c2.number_input("MVA/MPT", 240.0, min_value=10.0, max_value=2000.0, step=10.0)
    z_mpt = st.number_input("Impedance (pu)", 0.08, min_value=0.01, max_value=0.25, step=0.005, format="%.3f")
    xr_mpt = st.number_input("X/R", 40.0, min_value=1.0, max_value=100.0, step=1.0, key="xr_mpt")
    c3, c4 = st.columns(2)
    i0_mpt = c3.number_input("No-load I (pu)", 0.001, min_value=0.0, max_value=0.05, step=0.0005, format="%.4f")
    p0_mpt = c4.number_input("No-load P (kW)", 10.0, min_value=0.0, max_value=500.0, step=1.0)

    has_oltc = st.checkbox("Has OLTC tap changer?", value=True)
    if has_oltc:
        c5, c6 = st.columns(2)
        ntaps = c5.number_input("# Taps", 31, min_value=3, max_value=99, step=2)
        tap_range = c6.number_input("Range (%)", 10.0, min_value=1.0, max_value=30.0, step=1.0)
        ...
        fixed_tap = st.number_input("Fixed tap value (pu)", 0.025, min_value=-0.2, max_value=0.2, step=0.005, format="%.3f")

    st.markdown('<div class="section">Inverter Skid</div>', unsafe_allow_html=True)
    n_inverters = st.number_input("Number of Inverters", 53, min_value=1, max_value=1000)

    st.markdown("**Inverter Step-Up (ISU)**")
    c7, c8 = st.columns(2)
    s_isu = c7.number_input("MVA/ISU", 5.04, min_value=0.1, max_value=50.0, step=0.1)
    z_isu = c8.number_input("Imp. (pu)", 0.08, min_value=0.01, max_value=0.2, step=0.005, format="%.3f")
    c9, c10 = st.columns(2)
    xr_isu = c9.number_input("X/R ISU", 8.83, min_value=1.0, max_value=50.0, step=0.5)
    i0_isu = c10.number_input("NL I (pu)", 0.005, min_value=0.0, max_value=0.05, step=0.001, format="%.4f")
    p0_isu = st.number_input("NL losses/ISU (kW)", 8.0, min_value=0.0, max_value=100.0, step=1.0)

    st.markdown("**Inverter**")
    c11, c12 = st.columns(2)
    s_inv_mva = c11.number_input("MVA/inv", 5.0, min_value=0.1, max_value=50.0, step=0.5)
    pf_inv = c12.number_input("Power factor", 0.87, min_value=0.5, max_value=1.0, step=0.01)
    c13, c14 = st.columns(2)
    v_inv_max = c13.number_input("V max (pu)", 1.10, min_value=1.0, max_value=1.2, step=0.01)
    v_inv_min = c14.number_input("V min (pu)", 0.90, min_value=0.7, max_value=1.0, step=0.01)

    st.markdown('<div class="section">Capacitor Bank</div>', unsafe_allow_html=True)
    q_cap = st.number_input("Capacitor Bank (MVAR)", 0.0, min_value=0.0, max_value=500.0, step=5.0)
